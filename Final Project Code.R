...
#### Z-score Normalization #### # select numeric variables
SZ <- s_not_cancelled
...
########## Removing Outliers ################ Final_Dataset <- Z
...
#Z[Z$`Price Sensitivity`<ll2,]$`Price Sensitivity` <- ll2 No outliers
...
#Z[Z$`No of Flights p.a.`<ll3,]$`No of Flights p.a.` <- ll3 No outliers
...
#Z[Z$`No. of other Loyalty Cards`<ll4,]$`No. of other Loyalty Cards` <- ll4 No outliers
...
#Z[Z$`Shopping Amount at Airport`<ll5,]$`Shopping Amount at Airport` <- ll5 No outliers
...
#Z[Z$`Eating and Drinking at Airport`<ll6,]$`Eating and Drinking at Airport` <- ll6 No outliers
...
#Z[Z$`Departure Delay in Minutes`<ll7,]$`Departure Delay in Minutes` <- ll7 No outliers
...
# Z[Z$`Arrival Delay in Minutes`<ll8,]$`Arrival Delay in Minutes` <- ll8 No outliers
...
#Z[Z$Number_of_years<ll11,]$Number_of_years <- ll11 No outliers
#Z[Z$Number_of_years>ul11,]$Number_of_years <- ul11 No outliers
...
#Z[Z$`% of Flight with other Airlines`<ll12,]$`% of Flight with other Airlines` <- ll12 No outliers
Z[Z$`% of Flight with other Airlines`>ul12,]$`% of Flight with other Airlines` <- ul12

#### correlation chart#### str(s_not_cancelled) 
...
# We remove No. of other loyalty cards, Flight distance, departure delay in minutes due to multicollinearity issues as they are highly correlated with other explanatory variables
...
jitter(Z$`% of Flight with other Airlines`)
jitter(Z$`Flight time in minutes`) 
jitter(Z$Number_of_years)
jitter(Z$`Day of Month`)
jitter(Z$`Arrival Delay in Minutes`)

l1 <- lm(formula = Satisfaction ~ Age+`Price Sensitivity`+`No of Flights p.a.`+`Shopping Amount at Airport`+`Eating and Drinking at Airport`+`Arrival Delay in Minutes`+`Flight time in minutes`+Number_of_years+`Airline Status`+Gender+`Type of Travel`+Class+`Airline Name`+`Day of Month`+`% of Flight with other Airlines`, data = Z)
summary(l1)
...
l2 = lm(formula = Satisfaction ~ Age+`Price Sensitivity`+`Shopping Amount at Airport`+`Arrival Delay in Minutes`+`No of Flights p.a.`+`Airline Status`+Gender+`Type of Travel`+Class+`Airline Name`+Number_of_years, data = Z)
summary(l2)

# Removed airline names. However Adjusted R-squared goes down. So, keeping Airline names
l3 = lm(formula = Satisfaction ~ Age+`Price Sensitivity`+`Shopping Amount at Airport`+`Arrival Delay in Minutes`+`No of Flights p.a.`+`Airline Status`+Gender+`Type of Travel`+Class+Number_of_years, data = Z)
summary(l3)
...
#predict Average passengers (as defined by their overall satisfaction between 3 and 4). 
ruleset1 <- apriori(CX,parameter=list(support=0.1, confidence=0.3),appearance = list(rhs="Satisfaction=Average",default="lhs"))
...
ruleset2 <- apriori(CX,parameter=list(support=0.1, confidence=0.3),appearance = list(rhs="Satisfaction=Dissatisfaction",default="lhs"))
...
g + geom_bar(aes(fill=as$Satisfaction), width = 0.5) +theme(axis.text.x = element_text(angle=65, vjust=0.6)) + labs(title="satisfaction analysis", subtitle="Airline Status")
# Type of Travel
g <- ggplot(as, aes(as$`Type of Travel`))
g + geom_bar(aes(fill=as$Satisfaction), width = 0.5) +theme(axis.text.x = element_text(angle=65, vjust=0.6)) + labs(title="satisfaction analysis", subtitle="Type of Travel")
# Gender
g <- ggplot(as, aes(Gender))
g + geom_bar(aes(fill=Satisfaction), width = 0.5)+theme(axis.text.x = element_text(angle=65, vjust=0.6)) + labs(title="satisfaction analysis", subtitle="Gender")
# Airline Status
g <- ggplot(as, aes(`Airline Status`))
g + geom_bar(aes(fill=Satisfaction), width = 0.5) +theme(axis.text.x = element_text(angle=65, vjust=0.6)) + labs(title="satisfaction analysis", subtitle="Airline Status")
# Price Sensitivity
g <- ggplot(as, aes(`Price Sensitivity`))
g + geom_bar(aes(fill=Satisfaction), width = 0.5) +theme(axis.text.x = element_text(angle=65, vjust=0.6)) + labs(title="satisfaction analysis", subtitle="Price Sensitivity")

# Airline Name 
install.packages('ggthemes') 
library(ggthemes)
options(scipen = 999) # turns of scientific notations like 1e+40 # X Axis Breaks and Labels
brks <- seq(-15000000, 15000000, 5000000)
lbls = paste0(as.character(c(seq(15, 0, -5), seq(5, 15, 5))), "m") # Plot
...
ggplot(s_not_cancelled, aes(x = `Airline Name`, y = Satisfaction, fill = satisfaction)) +
geom_bar(stat = "identity", width = .6) + # draw the bars 
scale_y_continuous(breaks = brks, # Breaks
labels = lbls) + # Labels 
coord_flip() + # Flip axes
labs(title="Airline Company") + theme_tufte() + # Tufte theme from ggfortify 
...
cs + geom_bar(aes(fill=Satisfaction), col="black", width = 0.6) + theme(axis.text.x = element_text(angle=0, vjust=0.6)) + scale_fill_manual(values=c("Average"="#DFE1E3", "Satisfaction"="#798E9E", "Dissatisfaction"="#02353F")) + ggtitle("Satisfaction versus Class")
...
#############shopping
as2 <- s_not_cancelled
as2$Satisfaction <- as.factor(as2$Satisfaction)
ss <- ggplot(as2, aes(`Shopping Amount at Airport`)) + scale_fill_brewer(palette = "Spectral")
...
map1.us <- map1.us + coord_map() + labs(x="Latitude", y="Longitude", title="US Map by Satisfaction Level") + theme(plot.title = element_text(hjust = 0.5))
map1.us

########## Only for Southeast Airlines Co. ##########################
Satisfaction_Survey1 <- subset(Satisfaction_Survey, Satisfaction_Survey$`Airline Name` == "Southeast Airlines Co.")
Satisfaction_Survey1 <- subset(Satisfaction_Survey1, Satisfaction_Survey1$`Flight cancelled` == "No")
...
map2.us <- map2.us + coord_map() + labs(x="Latitude", y="Longitude", title="US Map by Satisfaction Level - Southeast Airlines Co.") + theme(plot.title = element_text(hjust = 0.5))
map2.us
...
dtree<-rpart(Satisfaction ~ Class +`Type of Travel`+`Airline Status`+ Age+ `Arrival Delay in Minutes` +`Price Sensitivity`+`Shopping Amount at Airport`+Gender
               ,data=traindata1, method="class", parms=list(split="information")) #Class+`Type of Travel`+`Airline Status`+Age+`Arrival Delay in Minutes`+`Price Sensitivity`+`Shopping Amount at Airport`+Gender
printcp(dtree)
